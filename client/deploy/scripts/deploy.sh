...
checkDockerImage() {
    ...
    if ! docker images | grep -q "${IMAGE_NAME}.*${IMAGE_TAG}"; then
        ...
        echo "Run: make build-client-service"
        exit 1
    fi
...
deployOrUpgrade() {
    ...
    if helm list -n "$NAMESPACE" | grep -q "$CHART_NAME"; then
        ...
        kubectl rollout restart deployment ${CHART_NAME}-client-chart -n "$NAMESPACE"
        
        echo "Waiting for rollout to complete..."
        kubectl rollout status deployment ${CHART_NAME}-client-chart -n "$NAMESPACE" --timeout=180s
    else
...
showDeploymentStatus() {
    ...
    echo "Pods:"
    kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=client-chart
    echo ""

    echo "Services:"
    kubectl get svc -n "$NAMESPACE" -l app.kubernetes.io/name=client-chart
    echo ""
...
showSummary() {
    ...
    echo "  kubectl logs -n $NAMESPACE -l app.kubernetes.io/name=client-chart"
    echo "  kubectl describe pod <pod-name> -n $NAMESPACE"
    echo ""
    echo "To update after changes:"
    echo "  make build-client-service && make deploy-client-service"
    echo ""
    echo "To remove:"
